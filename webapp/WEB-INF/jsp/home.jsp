            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <br>아쉬운 점은 포트폴리오를 준비하며 새로운 기능들을 반복학습하지 않고 너무 새로운 것만 추구했다는 점입니다.</p>
                        <h3>●과정</h3>
                        <ol>
                        ...
                        이 외에도 아이디 검색과 같은 원리로 닉네임 검색 기능도 넣었다. 내가 알던 회원가입을 최대한 구현하기 위해 노력했다. 이때 당시에는 오픈소스와 spring도 잘 몰랐기 때문에 하나하나 유효성 처리를 했었다. 로그인과 로그아웃은 구현하기 쉬웠다. 로그인을 해야지만 게시판 글쓰기가 가능하게 하였다. </p>
                        ...
                        <br>CSS에 대해서는 대략적으로만 알고 있는데 CSS보다는 JAVA를 더 알고싶어서 자세하게 들어가진 않았다. 초기 포트폴리오 메인은 사진과 같다.
                        <br><img src="/stylesheets/image/main.jpg" width="100%" onClick="window.open('/stylesheets/image/main.jpg')" style="cursor:hand" title="클릭시 원본파일을 보실 수 있습니다."></p>
                        ...
                        <br>후에 검색을 통해 그때그때 상황에 필요한 것을 찾아 사용했다.</p>
                        ...
                        <h3>●파일 업로드</h3>
                        <p>여기에 더해 파일 업로드까지 추가하기로 하였다. 어떻게 할지 고민하다가 배운 것을 활용하기로 했다. JAVA에서 FTP를 배웠기에 FTP를 이용하기로 했다. 호스팅으로 내 ftp 주소가 있기 때문에 ftp로 파일 업로드를 했다. ftp로 좀 돌아가는 것보다 FILE클래스로도 바로 파일을 생성하게 하는 것도 가능하리라 본다...</p>
                        <br>                        
                        ...
                        처음부터 본 것이 정말 행운이랄까? 어떻게 검색을 해야하고 처음 보는 오픈소스를 어떤 식으로 접근을 해야하는지 얽매여있던 나를 일깨우고 웹프로그래머로서 한단계 더 내디딜수 있는 정말 귀중한 시간이었다. 
                        ...
                        <p>동영상 강의를 다 보고나니 포트폴리오를 수정을 해야겠다는 생각을 하게되었다. bootstrap을 적용하기 위한 생각이기도 했지만 동영상 강의에서 배운 부분을 반복만 했지 따로 사용하지 않아 온전히 내 것이라고 하기엔 무리가 있어 배운 것을 활용하며 다시 만듬으로써 내 것이 되리라 생각해서 전면 다시 만들기로 하였다.<br>
						네이버 open api를 사용하고 다음에디터를 사용했다. 첫 포트폴리오에서도 AJAX전에 open api를 활용하려 했으나 이해가 가지 않아 다음으로 미뤘는데 지금의 나라면 가능할 것이라 생각해서 도전해 보았다. 물론 온전히 내 힘으로 했다고는 말 못하나 github을 통해 다른사람의 코드를 볼 수 있고 전에는 몰랐던 것을 아는 시간이었다.
                        </p>
                        ...
                        <p>SPRING-BOOT에서 다룬 내용인데 리눅스 서버를 좀 더 알아가는 시간이었다. 기억에 남는 건 1년간 무료 서비스를 받았는데 한달 씩 1년인줄 모르고 1달 넘게 나뒀다가 요금을 낸적이 있다. 다행히 AWS에서 이해를 해주고 결재금액을 다 돌려줌과 함께 따로 cash를 넣어주었다.</p>
                         ...
                         <h3>●TEST코드와 MyBatis 그리고 Mockito</h3>
                         ...
                         이전의 나는 테스트를 하기위해 TOMCAT을 실행하고 localhost로 접근하여 테스트를 해보며 잘못된 부분을 수정해 나갔는데 미리 테스트를 해봄으로써 번거로움을 덜었다. 물론 최종적으로는 사이트에서 확인을 해야겠지만 TEST코드의 존재는 이러한 편리함과 후에 작업해야하는 리팩토링에는 꼭 필요한 작업이라는 인식과 리팩토링에는 없어서는 안되는 부분이란 것을 알게되었다.
                         <br>MyBatis의 경우 새로운 기능에 어떤 식으로 접근을 해야하는지 알게 해주는 강의였는데 이 강의로 조금 더 spring을 알게 되는 강의였다.
                         <br>Mockito의 경우는 좀 더 연습하고 이해하기 위해서 아직 구현하지 않았다.</p>
                        ...
                        <h3>●다음에디터</h3>
                        <p>네이버 스마트에디터보다 다음에디터가 마음에 들었기에 다음에디터를 적용하였다.<br>
						OKKY사이트나 검색을 통해 자세히 설명이 되어있는 블로그를 보고 적용했다. <br>
						글자를 바꾸게 되면 태그가 적용되어 적게 쓰더라도 기존의 용량보다 더 큰 용량이 전송되어 에러가 났는데 BOARDS테이블의 contents 데이터 타입을 변경했다. </p>
                        
                        <h3>●Database Migration</h3>
                        <p>flyway db migration을 사용했다. 처음에는 게시판 글이 초기화되지 않았기 때문에 db migration이 실행되는 줄 알았는데 다음에디터를 적용하기 위해 column타입을 변경하지 않았더라면 계속 실행되는지 알았을 것이다. 배포할 때 플러그인이 실행되지 않으니 JAVA로 migrate가 실행되도록 하였다. 그래서 늦은 감이 있지만 플러그인에 대해 알아보는 시간을 갖고 궁금한 것을 풀어나갔다.
                        </p>
                        
                        <h3>●정규표현식</h3>
                        <p>누군가 회원가입 이름에 자바스크립트 alert를 저장했다. 그래서 이름을 불러오는 페이지에 알림창이 뜨는 현상이 나타났다.<br>
                        그분께 감사의 인사를 드린다. 크기를 줄이는 방법을 생각했다가 검색을 통해 hibernate validation이 정규표현식을 사용해도 된다는 것을 알고 적용했다.<br>
                        언젠가 할 것이라며 대충 차일피일 미루던 정규표현식. 사용하면서 이제는 조금 와닿았다. 그래서 더욱 감사를 드린다. 정말 재미있었다. 
                        </p>
                        ...
                        <ol>
                        	...
                        	<li>페이징 확인용으로 10개 이상의 글을 입력시켰습니다. 순서는 최신글 순입니다.(날짜 오름차순, 게시글 번호 오름차순)</li>
                        	<li>로그인을 하지 않았을 경우 글을 쓸 수 없습니다.</li>
                        	<li>글쓰기는 AJAX로 다음에디터를 적용하였습니다. (파일, 사진 업로드는 구현하지 않음)</li>
                        	<li>제목을 클릭하면 내용을 확인 할 수 있는 상세페이지로 이동합니다.</li>
                        	<li>댓글도 AJAX로 다음에디터를 적용하였습니다. AJAX로 댓글의 삭제기능도 구현하였습니다.</li>
                        	<li>아직 댓글의 수정기능은 구현하지 못했습니다.</li>
                        	<li>게시글을 쓴 이름과 로그인정보의 이름이 같으면 수정과 삭제가 가능합니다.</li>
                        	<li>만약에 개인정보에서 이름을 수정하였다면 게시글의 이름도 수정됩니다.</li>
                        </ol>
